...
const modules = [
  ...
  },
  {
    ...
    lessons: [
      { id: '6', title: '2.1: La API Key' },
    ]
  },
  {
    title: "Módulo 3: El Cerebro",
    lessons: [
      { id: '7', title: '3.1: Prompt Profesional' },
    ]
  },
  {
  ...
  },
  {
    ...
    lessons: [
      { id: '11', title: '5.1: ¡Lanzamiento y Futuro!' },
    ]
  }
];
...
export function CourseProgressSidebar() {
  ...
  return (
    <aside className="sticky top-8 h-full rounded-2xl border border-card-bg bg-card-bg/80 p-6 text-text-light shadow-lg shadow-black/20">
      ...
      <p className="mt-1 text-sm text-text-muted">Sigue tu avance y retoma exactamente donde lo dejaste.</p>
      <nav>
        <ul className="mt-6 space-y-5">
          {modules.map((module, moduleIndex) => (
            <li key={moduleIndex}>
              <h4 className="text-xs font-semibold uppercase tracking-[0.2em] text-text-muted">{module.title}</h4>
              <ul className="mt-3 space-y-1.5">
                {module.lessons.map(lesson => {
                  const lessonIndex = allLessons.findIndex(l => l.id === lesson.id);
                  const isCompleted = currentLessonIndex > -1 && lessonIndex < currentLessonIndex;
                    ...
                    <li key={lesson.id}>
                      <Link
                        ...
                        className={[
                          ...
                          isCurrent && 'border-text-accent/60 bg-text-accent/10 text-text-accent font-semibold',
                          !isCurrent && isCompleted && 'bg-emerald-500/10 text-emerald-200',
                          !isCurrent && !isCompleted && 'hover:border-text-muted/40 hover:bg-black/40 text-text-muted'
                        ].filter(Boolean).join(' ')}
                      >
