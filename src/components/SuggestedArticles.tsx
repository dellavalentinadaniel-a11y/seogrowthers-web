...
export default function SuggestedArticles({ currentPost, allPosts, limit = DEFAULT_LIMIT }: SuggestedArticlesProps) {
  ...
  const suggestions = related.length >= limit ? related : [...related, ...candidates]
  ...
  return (
    <section className="mt-12 rounded-2xl border border-card-bg bg-card-bg/80 p-6 shadow-lg shadow-black/20">
      <h2 className="text-2xl font-semibold text-white text-center">También te puede interesar</h2>
      <div className="mt-8 grid grid-cols-1 gap-6 md:grid-cols-3">
        {suggestions.map((post) => (
          ...
          >
            <h3 className="text-lg font-semibold text-white group-hover:text-text-accent">{post.title}</h3>
            <p className="mt-3 flex-grow text-sm text-text-muted line-clamp-3">{post.description}</p>
            <div className="mt-4 flex flex-wrap gap-2">
              {post.tags.slice(0, 2).map((tag) => (
                <span key={tag} className="rounded-full bg-black/40 px-2 py-1 text-xs text-text-muted">
                  #{tag}
                </span>
              ))}
            </div>
